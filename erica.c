...
int load_src(char *file) {
    ...
    if (!(src = src_dump = malloc(MAX_SIZE))) {
        printf("could not malloc (%lld) memory for source %s", MAX_SIZE, file);
        return -1;
    }
...
void tokenize() {
    ...
    while (token = *src++) {
        ...
        // handle string & char
            ...
            src++;
            // judge string or char
            if (token == '"')
                token_val = (int)ch_ptr;
            // mark char as integer
            else
                token = Num;
            return ;
        }
        // handle comments or Div
        else if (token == '/') {
            if (*src == '/') {
                while (*src != 0 && *src != '\n')
                    src++;
...
// check token is tk and take next token
void assert(int tk) {
    if (token != tk) {
        printf("line %lld: expect token: %lld(%c), get: %lld(%c)", line , tk, (char)tk, token, (char)token);
        exit(-1);
    }
    tokenize();
}
...
int32_t main(int32_t argc, char **argv) {
    ...
    // init symbol table
    keyword();
    // parse and generate vm instructions
    parse();
